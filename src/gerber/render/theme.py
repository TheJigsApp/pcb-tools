...
class Theme(object):
    def __init__(self, name=None, **kwargs):
        ...
        self.bottomsilk = kwargs.get(
            "bottomsilk", RenderSettings(COLORS["white"], mirror=True)
        )
        self.topmask = kwargs.get(
            "topmask",
            RenderSettings(COLORS["green soldermask"], alpha=0.85, invert=True),
        )
        self.bottommask = kwargs.get(
            "bottommask",
            RenderSettings(
                COLORS["green soldermask"], alpha=0.85, invert=True, mirror=True
            ),
        )
        self.top = kwargs.get("top", RenderSettings(COLORS["hasl copper"]))
        self.bottom = kwargs.get(
...
THEMES = {
    ...
    "OSH Park": Theme(
        ...
        bottommask=RenderSettings(
            COLORS["purple soldermask"], alpha=0.85, invert=True, mirror=True
        ),
    ...
    "Transparent Multilayer": Theme(
        name="Transparent Multilayer",
