...
def rotate_point(point, angle, center=(0.0, 0.0)):
    ...
    return (
        cos_angle * (point[0] - center[0])
        - sin_angle * (point[1] - center[1])
        + center[0],
        sin_angle * (point[0] - center[0])
        + cos_angle * (point[1] - center[1])
        + center[1],
    )
...
def nearly_equal(point1, point2, ndigits=6):
    ...
    return (
        round(point1[0] - point2[0], ndigits) == 0
        and round(point1[1] - point2[1], ndigits) == 0
    )
...
def listdir(directory, ignore_hidden=True, ignore_os=True):
    ...
    if ignore_os:
        files = [f for f in files if f not in os_files]
    return files
...
def ConvexHull_qh(points):
    ...
    hull = (
        [minxp]
        + _findhull(rpoints, maxxp, minxp, points)
        + [maxxp]
        + _findhull(lpoints, minxp, maxxp, points)
    )
...
def _optimize(hull, points):
    ...
    for i in range(l):
        ...
        if (points[p1][1] - points[p2][1]) * (points[p1][0] - points[p3][0]) == (
...
def _distance(a, b, x):
    ...
    return abs(
        (b[1] - a[1]) * x[0] - (b[0] - a[0]) * x[1] + b[0] * a[1] - a[0] * b[1]
    ) / sqrt((b[1] - a[1]) ** 2 + (b[0] - a[0]) ** 2)
...
def _findhull(idxp, a_i, b_i, points):
    ...
    ret = (
        _findhull(rpoints, farpoint, b_i, points)
        + [farpoint]
        + _findhull(lpoints, a_i, farpoint, points)
    )
    return ret
