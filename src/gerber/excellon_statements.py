...
__all__ = [
    ...
    "CutterCompensationOffStmt",
    "CutterCompensationLeftStmt",
    "CutterCompensationRightStmt",
    "ZAxisInfeedRateStmt",
...
class ExcellonTool(ExcellonStatement):
    ...
    def from_excellon(cls, line, settings, id=None, plated=None):
        ...
        for cmd, val in commands:
            if cmd == "B":
                args["retract_rate"] = parse_gerber_value(
                    val, nformat, zero_suppression
                )
            elif cmd == "C":
                ...
                args["max_hit_count"] = parse_gerber_value(
                    val, nformat, zero_suppression
                )
            elif cmd == "S":
                ...
                args["depth_offset"] = parse_gerber_value(
                    val, nformat, zero_suppression
                )
    ...
    def to_excellon(self, settings=None):
        ...
        if self.rpm is not None:
            if self.rpm < 100000.0:
                stmt += "S%s" % write_gerber_value(self.rpm / 1000.0, fmt, zs)
            else:
                stmt += "S%g" % (self.rpm / 1000.0)
...
class ToolSelectionStmt(ExcellonStatement):
    ...
    def __init__(self, tool, compensation_index=None, **kwargs):
        ...
        tool = int(tool)
        compensation_index = (
            int(compensation_index) if compensation_index is not None else None
        )
        self.tool = tool
...
class CoordinateStmt(ExcellonStatement):
    ...
    def from_excellon(cls, line, settings, **kwargs):
        ...
        if line[0] == "X":
            ...
            x_coord = parse_gerber_value(
                splitline[0], settings.format, settings.zero_suppression
            )
            if len(splitline) == 2:
                y_coord = parse_gerber_value(
                    splitline[1], settings.format, settings.zero_suppression
                )
        else:
            y_coord = parse_gerber_value(
                line.strip(" Y"), settings.format, settings.zero_suppression
            )
...
class RepeatHoleStmt(ExcellonStatement):
    ...
    def from_excellon(cls, line, settings, **kwargs):
        match = re.compile(
            r"R(?P<rcount>[0-9]*)X?(?P<xdelta>[+\-]?\d*\.?\d*)?Y?"
            "(?P<ydelta>[+\-]?\d*\.?\d*)?"
        ).match(line)
        ...
        xdelta = (
            parse_gerber_value(
                stmt["xdelta"], settings.format, settings.zero_suppression
            )
            if stmt["xdelta"] != ""
            else None
        )
        ydelta = (
            parse_gerber_value(
                stmt["ydelta"], settings.format, settings.zero_suppression
            )
            if stmt["ydelta"] != ""
            else None
        )
    ...
    def to_excellon(self, settings):
        ...
        if self.xdelta is not None and self.xdelta != 0.0:
            stmt += "X%s" % write_gerber_value(
                self.xdelta, settings.format, settings.zero_suppression
            )
        if self.ydelta is not None and self.ydelta != 0.0:
            stmt += "Y%s" % write_gerber_value(
                self.ydelta, settings.format, settings.zero_suppression
            )
        return stmt
...
class EndOfProgramStmt(ExcellonStatement):
    ...
    def from_excellon(cls, line, settings, **kwargs):
        ...
        stmt = match.groupdict()
        x = (
            parse_gerber_value(stmt["x"], settings.format, settings.zero_suppression)
            if stmt["x"] != ""
            else None
        )
        y = (
            parse_gerber_value(stmt["y"], settings.format, settings.zero_suppression)
            if stmt["y"] != ""
            else None
        )
        c = cls(x, y, **kwargs)
...
class SlotStmt(ExcellonStatement):
    ...
    def from_excellon(cls, line, settings, **kwargs):
        ...
        (x_start_coord, y_start_coord) = SlotStmt.parse_sub_coords(
            sub_coords[0], settings
        )
    ...
    def to_excellon(self, settings):
        ...
        if self.x_start is not None:
            stmt += "X%s" % write_gerber_value(
                self.x_start, settings.format, settings.zero_suppression
            )
        if self.y_start is not None:
            stmt += "Y%s" % write_gerber_value(
                self.y_start, settings.format, settings.zero_suppression
            )
        ...
        if self.x_end is not None:
            stmt += "X%s" % write_gerber_value(
                self.x_end, settings.format, settings.zero_suppression
            )
        if self.y_end is not None:
            stmt += "Y%s" % write_gerber_value(
                self.y_end, settings.format, settings.zero_suppression
            )
