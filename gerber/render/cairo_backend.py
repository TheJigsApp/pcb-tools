...
class GerberCairoContext(GerberContext):
    ...
    def origin_in_pixels(self):
        return (
            self.scale_point(self.origin_in_inch)
            if self.origin_in_inch is not None
            else (0.0, 0.0)
        )
    ...
    def size_in_pixels(self):
        return (
            self.scale_point(self.size_in_inch)
            if self.size_in_inch is not None
            else (0.0, 0.0)
        )
    ...
    def set_bounds(self, bounds, new_surface=False):
        ...
        self.origin_in_inch = (
            origin_in_inch if self.origin_in_inch is None else self.origin_in_inch
        )
        self.size_in_inch = (
            size_in_inch if self.size_in_inch is None else self.size_in_inch
        )
        self._xform_matrix = cairo.Matrix(
            xx=1.0, yy=-1.0, x0=-self.origin_in_pixels[0], y0=self.size_in_pixels[1]
        )
    ...
    def render_layers(
        ...
        for layer in layers:
            ...
            self.render_layer(
                layer, settings=settings, bgsettings=bgsettings, verbose=verbose
            )
        self.dump(filename, verbose)
    ...
    def _new_mask(self):
        class Mask:
            def __enter__(msk):
                ...
                msk.surface = cairo.SVGSurface(
                    None, size_in_pixels[0], size_in_pixels[1]
                )
    ...
    def _render_line(self, line, color):
        ...
        self.ctx.set_operator(
            cairo.OPERATOR_OVER
            if (not self.invert) and line.level_polarity == "dark"
    ...
    def _render_arc(self, arc, color):
        ...
        self.ctx.set_operator(
            cairo.OPERATOR_OVER
            if (not self.invert) and arc.level_polarity == "dark"
            else cairo.OPERATOR_CLEAR
        )
        with self._clip_primitive(arc):
            with self._new_mask() as mask:
                mask.ctx.set_line_width(width * self.scale[0])
                mask.ctx.set_line_cap(
                    cairo.LINE_CAP_ROUND
                    if isinstance(arc.aperture, Circle)
                    else cairo.LINE_CAP_SQUARE
                )
                ...
                for prim in region.primitives:
                    if isinstance(prim, Line):
                        ...
                        if prim.direction == "counterclockwise":
                            ...
                            mask.ctx.arc_negative(
                                center[0], center[1], radius, angle1, angle2
                            )
                mask.ctx.fill()
                self.ctx.mask_surface(mask.surface, self.origin_in_pixels[0])
    ...
    def _render_circle(self, circle, color):
        ...
        self.ctx.set_operator(
            cairo.OPERATOR_OVER
            if (not self.invert) and circle.level_polarity == "dark"
            else cairo.OPERATOR_CLEAR
        )
        with self._clip_primitive(circle):
            with self._new_mask() as mask:
                ...
                if (
                    hasattr(circle, "hole_diameter")
                    and circle.hole_diameter is not None
                    and circle.hole_diameter > 0
                ):
                    mask.ctx.set_operator(cairo.OPERATOR_CLEAR)
                    mask.ctx.arc(
                        center[0],
                        center[1],
                        circle.hole_radius * self.scale[0],
                        0,
                        2 * math.pi,
                    )
                    mask.ctx.fill()
                ...
                if (
                    hasattr(circle, "hole_width")
                    and hasattr(circle, "hole_height")
                    and circle.hole_width is not None
                    and circle.hole_height is not None
                    and circle.hole_width > 0
                    and circle.hole_height > 0
                ):
                    mask.ctx.set_operator(
                        cairo.OPERATOR_CLEAR
                        if circle.level_polarity == "dark" and (not self.invert)
                        else cairo.OPERATOR_OVER
                    )
                    width, height = self.scale_point(
                        (circle.hole_width, circle.hole_height)
                    )
                    ...
                    lower_right = rotate_point(
                        (center[0] + width / 2.0, center[1] - height / 2.0),
                        circle.rotation,
                        center,
                    )
                    upper_left = rotate_point(
                        (center[0] - width / 2.0, center[1] + height / 2.0),
                        circle.rotation,
                        center,
                    )
                    upper_right = rotate_point(
                        (center[0] + width / 2.0, center[1] + height / 2.0),
                        circle.rotation,
                        center,
                    )
    ...
    def _render_rectangle(self, rectangle, color):
        ...
        width, height = tuple(
            [
                abs(coord)
                for coord in self.scale_point((rectangle.width, rectangle.height))
            ]
        )
        self.ctx.set_operator(
            cairo.OPERATOR_OVER
            if (not self.invert) and rectangle.level_polarity == "dark"
            else cairo.OPERATOR_CLEAR
        )
            ...
            with self._new_mask() as mask:
                ...
                if rectangle.hole_diameter > 0:
                    ...
                    mask.ctx.arc(
                        center[0],
                        center[1],
                        rectangle.hole_radius * self.scale[0],
                        0,
                        2 * math.pi,
                    )
                    mask.ctx.fill()
    ...
    def _render_obround(self, obround, color):
        ...
        with self._clip_primitive(obround):
            with self._new_mask() as mask:
                ...
                # Render circles
                for circle in (
                    obround.subshapes["circle1"],
                    obround.subshapes["circle2"],
                ):
                    center = self.scale_point(circle.position)
                    mask.ctx.arc(
                        center[0],
                ...
                if obround.hole_diameter > 0:
                    ...
                    mask.ctx.arc(
                        center[0],
                        center[1],
                        obround.hole_radius * self.scale[0],
                        0,
                        2 * math.pi,
                    )
                    mask.ctx.fill()
                ...
                if obround.hole_width > 0 and obround.hole_height > 0:
                    ...
                    width, height = self.scale_point(
                        (obround.hole_width, obround.hole_height)
                    )
                    lower_left = rotate_point(
                        (center[0] - width / 2.0, center[1] - height / 2.0),
                        obround.rotation,
            ...
            with self._new_mask() as mask:
                ...
                if polygon.hole_radius > 0:
                    ...
                    mask.ctx.arc(
                        center[0],
                        center[1],
                        polygon.hole_radius * self.scale[0],
                        0,
                        2 * math.pi,
                    )
                    mask.ctx.fill()
