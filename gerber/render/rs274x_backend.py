...
from ..primitives import AMGroup, Circle, Line, Obround, Outline, Polygon, Rectangle
...
class Rs274xContext(GerberContext):
    ...
    def _render_arc(self, arc, color, default_polarity="dark"):
        ...
        if self._pos != arc.start:
            ...
            self.body.append(
                CoordStmt.move(CoordStmt.FUNC_LINEAR, self._simplify_point(arc.start))
            )
            self._pos = arc.start
    ...
    def _render_flash(self, primitive, aperture):
        ...
        if self.condensed_flash:
            ...
            self.body.append(
                CoordStmt.move(None, self._simplify_point(primitive.position))
            )
    ...
    def _get_circle(
        ...
        if not aper:
            ...
            aper = ADParamStmt.circle(
                dcode, diameter, hole_diameter, hole_width, hole_height
            )
    ...
    def _get_rectangle(
        ...
        if not aper:
            ...
            aper = ADParamStmt.rect(
                dcode, width, height, hole_diameter, hole_width, hole_height
            )
    ...
    def _render_rectangle(self, rectangle, color):
        aper = self._get_rectangle(
            ...
            rectangle.hole_width,
            rectangle.hole_height,
        )
    ...
    def _get_amacro(self, amgroup, dcode=None):
        ...
        if macroinfo:
            ...
            for macro in macroinfo:
                ...
                offset = (
                    position[0] - macro[1].position[0],
                    position[1] - macro[1].position[1],
                )
