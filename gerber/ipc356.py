...
class IPCNetlist(CamFile):
    ...
    def __init__(self, statements, settings, primitives=None, filename=None):
        ...
        self.primitives = [
            TestRecord((rec.x_coord, rec.y_coord), rec.net_name, rec.access)
    ...
    def comments(self):
        return [
            record for record in self.statements if isinstance(record, IPC356_Comment)
        ]

    @property
    def parameters(self):
        return [
            record for record in self.statements if isinstance(record, IPC356_Parameter)
        ]
    ...
    def nets(self):
        nets = []
        for net in list(
            set([rec.net_name for rec in self.test_records if rec.net_name is not None])
        ):
            adjacent_nets = set()
    ...
    @property
    def outlines(self):
        return [stmt for stmt in self.statements if isinstance(stmt, IPC356_Outline)]
...
class IPC356_TestRecord(object):
    ...
    def from_line(cls, line, settings):
        ...
        record["pin"] = (
            line[27 + offset : end].strip()
            if line[27 + offset : end].strip() != ""
            else None
        )
        ...
        if line[32 + offset] == "D":
            ...
            record["hole_diameter"] = dia * 0.0001 if units == "inch" else dia * 0.001
        ...
        if len(line) >= (43 + offset):
            ...
            record["x_coord"] = coord * 0.0001 if units == "inch" else coord * 0.001

        if len(line) >= (51 + offset):
            end = len(line) - 1 if len(line) < (58 + offset) else (57 + offset)
            coord = int(line[50 + offset : end].strip())
            record["y_coord"] = coord * 0.0001 if units == "inch" else coord * 0.001
        ...
        if len(line) >= (59 + offset):
            ...
            if dim != "":
                record["rect_x"] = (
                    int(dim) * 0.0001 if units == "inch" else int(dim) * 0.001
                )
        ...
        if len(line) >= (69 + offset):
            ...
            if rot != "":
                record["rect_rotation"] = (
                    int(rot) if angle == "degrees" else math.degrees(rot)
                )
...
class IPC356_Conductor(object):
    ...
    def from_line(cls, line, settings):
        ...
        x = 0
        y = 0
        x = int(aperture_dict["x"]) * scale if aperture_dict["x"] != "" else None
        y = int(aperture_dict["y"]) * scale if aperture_dict["y"] != "" else None
        aperture = (x, y)
